...
export class AuthService {
  ...
  async signIn(authCredentials: AuthCredentialsDto, res: Response) {
    ...
    res.status(HttpStatus.OK).json({ accessToken, username });
  }

  //checked.
  async refresh(jwt: string) {
    if (!jwt) {
      throw new UnauthorizedException('Unauthorized no jwt cookie provided!');
    }
    ...
    try {
      ...
      const newAccessToken = this.createJwtAccessToken(payload, '1h');

      return { accessToken: newAccessToken, username: payload };
    } catch (error) {
